...
export default function ManageCarTypes() {
  ...
  const handleAdd = async (e) => {
    ...
    const nameErr = validateCarTypeName(formState.newName, carTypes)
    if (nameErr) {
      formDispatch({ type: 'SET_UNIQUE_ERROR', payload: true })
      return
    }
    try {
      const added = await addCarType({ name: formState.newName })
      setCarTypes((prev) => [...prev, added])
  ...
  const handleDeleteClick = async (id, name) => {
    ...
    try {
      ...
      if (inUse) {
        setDeleteError(`Cannot delete ${name} because it is currently in use.`)
      } else {
        setDeleteError(null)
      }
    } catch (err) {
      setDeleteError(`Error checking: ${err.message}`)
    }
    setShowModal(true)
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      alert(err.message || 'Failed to delete car type.')
    }
  }
